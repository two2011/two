...
public class ReflexiveCommandFactory implements CommandFactory,
    ...
    private Command createCommandInstance(String commandId) {
        Class<? extends Command> commandClass = commandClassRegistry
                .getCommandClass(commandId);
        Command command = applicationContext.getBean(commandClass);
        return command;
    }

    private void injectCommandParameters(Command command,
            Map<String, String> parametersValuesByName) {
        ...
        for (Field parameterField : parametersFields) {
            ...
            injectFieldValue(command, parameterField,
                    parametersValuesByName.get(fieldName));
        }

    }

    private void assertParameterPresentInTheMap(String fieldName,
            Map<String, String> parametersValuesByName) {
        if (!parametersValuesByName.containsKey(fieldName)) {
            logger.error(String
                    .format("Required command parameter \"%s\" not found in the parameters map.",
                            fieldName));
            throw new NoSuchElementException();
        }
    }

    private void injectFieldValue(Command command, Field parameterField,
            String parameterValue) {
        Object parsedParameter = parseParameter(parameterField.getType(),
                parameterValue);
        try {
    ...
    public void setCommandClassRegistry(
            CommandClassRegistry commandClassRegistry) {
